...
// Private functions
void uart_send_packet_no_fwd(unsigned char *data, unsigned int len);
...
void uart_bldc_interface_terminal_cmd(char* cmd) {
	...
	uart_send_packet_no_fwd(send_buffer, len + 1);
}
...
void uart_bldc_interface_set_duty_cycle(float dutyCycle) {
	...
	uart_send_packet_no_fwd(send_buffer, send_index);
}
...
void uart_bldc_interface_set_mcconf(const mc_configuration *mcconf) {
	...
	uart_send_packet_no_fwd(send_buffer, ind);
}
...
void uart_send_packet_no_fwd(unsigned char *data, unsigned int len) {
	if (!forward_func) {
		uart_bldc_interface_send_packet(data, len);
	}
}
